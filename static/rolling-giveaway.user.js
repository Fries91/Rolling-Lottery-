...
(function () {
  ...
  const UI_SIZE_KEY = "fries91_giveaway_ui_size_v1";
  const UI_SIZES = ["compact", "normal", "large"];

  function getUiSize() {
    const saved = localStorage.getItem(UI_SIZE_KEY) || "normal";
    return UI_SIZES.includes(saved) ? saved : "normal";
  }

  function applyUiSize() {
    const size = getUiSize();
    document.documentElement.classList.remove("fg-ui-compact", "fg-ui-normal", "fg-ui-large");
    document.documentElement.classList.add("fg-ui-" + size);
    document.querySelectorAll(".fg-size-reset").forEach(btn => {
      btn.textContent = size === "compact" ? "Small" : size === "large" ? "Large" : "Normal";
      btn.title = "Current size: " + btn.textContent;
    });
  }

  function setUiSize(direction) {
    const current = getUiSize();
    let idx = UI_SIZES.indexOf(current);
    if (direction === "smaller") idx = Math.max(0, idx - 1);
    else if (direction === "larger") idx = Math.min(UI_SIZES.length - 1, idx + 1);
    else idx = 1;
    localStorage.setItem(UI_SIZE_KEY, UI_SIZES[idx]);
    applyUiSize();
  }
  ...
  function ensurePanel() {
    ...
    panel.innerHTML = `
      <div class="fg-head">
        ...
        <div class="fg-head-actions">
          <div class="fg-size-controls" title="PC size controls">
            <button class="fg-size-btn fg-size-down" type="button" title="Make smaller">−</button>
            <button class="fg-size-btn fg-size-reset" type="button" title="Reset size">Normal</button>
            <button class="fg-size-btn fg-size-up" type="button" title="Make bigger">+</button>
          </div>
          <button class="fg-close" type="button">×</button>
        </div>
      </div>
      <div class="fg-tabs">
    ...
    $(".fg-size-down", panel)?.addEventListener("click", (ev) => { ev.stopPropagation(); setUiSize("smaller"); });
    $(".fg-size-up", panel)?.addEventListener("click", (ev) => { ev.stopPropagation(); setUiSize("larger"); });
    $(".fg-size-reset", panel)?.addEventListener("click", (ev) => { ev.stopPropagation(); setUiSize("normal"); });
...
$("#fg-go-rules-login")?.addEventListener("click", () => {
  ...
  GM_addStyle(`
    ...
    .fg-head { display:flex; align-items:center; justify-content:space-between; gap:10px; padding: 14px; background: linear-gradient(135deg,#1b102b,#301a50); cursor: move; touch-action: none; }
    .fg-head-left { min-width:0; }
    .fg-head-actions { display:flex; align-items:center; gap:8px; flex:0 0 auto; }
    .fg-size-controls { display:flex; align-items:center; gap:4px; padding:3px; border-radius:10px; background:rgba(0,0,0,.22); border:1px solid rgba(255,255,255,.12); }
    .fg-size-btn { min-width:26px; height:24px; padding:0 7px; border-radius:8px; border:1px solid rgba(255,255,255,.16); background:#202333; color:#fff; font-size:12px; font-weight:900; cursor:pointer; }
    .fg-size-reset { min-width:54px; }
    #fries-giveaway-panel.fg-dragging { user-select: none !important; opacity: .96; }
    .fg-title { font-weight: 800; font-size: 18px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    ...
    .fg-body { padding: 12px 12px 88px; overflow-y:auto; overflow-x:hidden; max-height: calc(100vh - 180px); min-height:0; -webkit-overflow-scrolling: touch; overscroll-behavior: contain; }
    #fries-giveaway-panel.open { display: flex; flex-direction: column; }
    ...
    @media (min-width: 721px) {
      html.fg-ui-compact #fries-giveaway-page-header { width: min(470px, calc(100vw - 430px)) !important; min-width: 360px !important; }
      html.fg-ui-compact #fries-giveaway-topbar { min-height: 30px !important; padding: 4px 8px !important; gap: 6px !important; }
      html.fg-ui-compact .fg-top-icon { width: 20px !important; height: 20px !important; font-size: 12px !important; }
      html.fg-ui-compact .fg-top-text { font-size: 11px !important; letter-spacing: .06em !important; }
      html.fg-ui-compact .fg-top-marquee { font-size: 10px !important; }
      html.fg-ui-compact #fries-giveaway-panel { width: min(500px, calc(100vw - 24px)) !important; top: 94px !important; }
      html.fg-ui-compact .fg-head { padding: 10px 12px !important; }
      html.fg-ui-compact .fg-title { font-size: 15px !important; }
      html.fg-ui-compact .fg-sub { font-size: 10px !important; }
      html.fg-ui-compact .fg-tabs { padding: 8px !important; gap: 5px !important; }
      html.fg-ui-compact .fg-tabs button { padding: 7px 6px !important; font-size: 12px !important; }
      html.fg-ui-compact .fg-body { padding: 9px 9px 88px !important; max-height: calc(100vh - 190px) !important; }
      html.fg-ui-compact .fg-card { padding: 9px !important; margin-bottom: 8px !important; }
      html.fg-ui-compact .fg-hero { padding: 24px 14px 14px !important; }
      html.fg-ui-compact .fg-hero h2 { font-size: 19px !important; }
      html.fg-ui-compact .fg-big { font-size: 26px !important; }
      html.fg-ui-compact .fg-input, html.fg-ui-compact .fg-primary, html.fg-ui-compact .fg-secondary, html.fg-ui-compact .fg-warn { padding: 8px !important; }

      html.fg-ui-large #fries-giveaway-page-header { width: min(760px, calc(100vw - 430px)) !important; min-width: 560px !important; }
      html.fg-ui-large #fries-giveaway-topbar { min-height: 44px !important; padding: 9px 12px !important; }
      html.fg-ui-large .fg-top-icon { width: 28px !important; height: 28px !important; font-size: 17px !important; }
      html.fg-ui-large .fg-top-text { font-size: 15px !important; }
      html.fg-ui-large .fg-top-marquee { font-size: 13px !important; }
      html.fg-ui-large #fries-giveaway-panel { width: min(760px, calc(100vw - 24px)) !important; top: 112px !important; }
      html.fg-ui-large .fg-head { padding: 16px !important; }
      html.fg-ui-large .fg-title { font-size: 21px !important; }
      html.fg-ui-large .fg-sub { font-size: 13px !important; }
      html.fg-ui-large .fg-tabs button { padding: 11px 9px !important; font-size: 15px !important; }
      html.fg-ui-large .fg-body { padding: 14px 14px 96px !important; }
      html.fg-ui-large .fg-hero h2 { font-size: 26px !important; }
      html.fg-ui-large .fg-big { font-size: 38px !important; }
    }
    ...
    @media (min-width: 721px) {
      ...
      .fg-body { max-height: calc(100vh - 220px) !important; padding-bottom: 96px !important; }
    }
    @media (max-width: 720px) {
      .fg-size-controls { display:none !important; }
      #fries-giveaway-panel {
        left: 8px !important;
        right: 8px !important;
        top: 86px !important;
  ...
  window.addEventListener("resize", applyUiSize, { passive: true });
})();
