...
$("#fg-go-rules-login")?.addEventListener("click", () => {
  ...
  function renderRules() {
    ...
    $(".fg-body").innerHTML = `
      ...
      <div class="fg-card">
        ...
        <p>Use a Limited Torn API key for this app. Do not use your Torn password. If login fails, clear the saved key, make a fresh Limited key, and paste it with no spaces.</p>
      </div>
    ...
    $("#fg-rules-login")?.addEventListener("click", async () => {
      const key = $("#fg-rules-api-key").value.replace(/\s+/g, "").trim();
  ...
  GM_addStyle(`
    ...
    #fries-giveaway-panel { position: fixed; right: 12px; top: 46px; z-index: 999999; width: min(430px, calc(100vw - 24px)); max-height: calc(100dvh - 64px); display: none; flex-direction: column; overflow: hidden; border-radius: 18px; background: #11131a; color: #f4f2ff; border: 1px solid rgba(255,255,255,.16); box-shadow: 0 18px 60px rgba(0,0,0,.55); font-family: Arial, sans-serif; }
    #fries-giveaway-panel.open { display: flex; }
    ...
    .fg-body { padding: 12px 12px 110px; overflow-y:auto; overflow-x:hidden; flex: 1 1 auto; min-height: 0; max-height: none; -webkit-overflow-scrolling: touch; overscroll-behavior: contain; scrollbar-gutter: stable; }
    ...
    @media (max-width: 520px) {
      ...
      #fries-giveaway-panel { right: 8px; left: 8px; width: auto; top: 48px; max-height: calc(100dvh - 64px); }
      .fg-body { padding-bottom: 120px; }
